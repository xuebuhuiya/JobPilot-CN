...
2026-09-16 更新：已完成 9 条上海 BOSS 岗位的真实采集与评分；现已实现独立资格/JD质量检查、SQLite 统一岗位池及人工审阅/申请状态记录，13 项测试通过。原始分数保留，不能覆盖资格问题。

扩大到约30条的测试遇到 BOSS 验证码，已停止，尚未完成。当前岗位池入口为命令行和 Markdown 报告，未嵌入 BossHunter 页面，也未实现完整自动填表或投递。见 [使用说明](docs/job-pool.md) 和 [实现验收](docs/implementation-status-20260916.md)。
